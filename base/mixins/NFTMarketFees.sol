  ...
  SendValueWithFallbackWithdraw
{
  ...
  event MarketFeesUpdated(
    ...
    uint256 secondaryMarketFeeBasisPoints,
    uint256 secondaryCreatorFeeBasisPoints
  );

  uint256 private _primaryMarketFeeBasisPoints;
  uint256 private _secondaryMarketFeeBasisPoints;
  ...
  {
    return (_primaryMarketFeeBasisPoints, _secondaryMarketFeeBasisPoints, _secondaryCreatorFeeBasisPoints);
  }
    ...
    )
  {
    ...
    if (_getIsPrimary(nftContract, tokenId, creator, seller)) {
      foundationFeeBasisPoints = _primaryMarketFeeBasisPoints;
      // On a primary sale, the creator is paid the remainder via `ownerRev`.
      ownerRevTo = tokenCreatorPaymentAddress;
    } else {
      foundationFeeBasisPoints = _secondaryMarketFeeBasisPoints;
    ...
    )
  {
    ...
    _sendValueWithFallbackWithdrawWithLowGasLimit(getTreasury(), foundationFee);
